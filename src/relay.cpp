...
static void relayTask(void*) {
  for (;;) {
    if (!s_active || WiFi.status() != WL_CONNECTED) { vTaskDelay(pdMS_TO_TICKS(400)); continue; }
    String cmd = pullOne();
    if (cmd.length()) { RelayMsg m; strlcpy(m.s, cmd.c_str(), sizeof(m.s)); xQueueSend(s_cmdQ, &m, 0); }
